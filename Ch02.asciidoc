...
Coin set model is based on UTXO which only keeps track of unspent coins/value. The transaction model keeps track of all transactions within accounts and generates a balances for specific account on the fly. Because coin set model is simpler it's generally considered more secure. 
